...
class master_registrations extends database{
  ...
  public function index($course_code){
    ...
    $registrations_index['registrations'] = parent::selectQuery(
    array("query" => "SELECT CONCAT(?, mc.course_code, '-',LPAD(RIGHT(mr.reg_no, 6) + 1, 6, '0')) reg_no
    FROM `master_registrations` mr INNER JOIN `master_branches` mb ON mr.branch_id = mb.id
    INNER JOIN `master_courses` mc ON mc.id = mr.course_id WHERE mc.course_code = ?
    ORDER BY mr.reg_no DESC LIMIT 1",
  "data" => array(BRANCH_CODE, $course_code)
));

$registrations_index['courses'] = parent::selectQuery(
array("query" => "SELECT mc.id,mc.course_code, mc.course_name FROM `master_courses` mc
